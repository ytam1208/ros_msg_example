...
#include <value_msg/AveragingAction.h>

class AveragingAction
{
public:
    AveragingAction(std::string name) : as_(nh_, name, false),
                                        action_name_(name)
    {
...
protected:
    ros::NodeHandle nh_;
    actionlib::SimpleActionServer<value_msg::AveragingAction> as_;
    std::string action_name_;
    int data_count_, goal_;
    float sum_, sum_sq_;
    value_msg::AveragingFeedback feedback_;
    value_msg::AveragingResult result_;
    ros::Subscriber sub_;
};
